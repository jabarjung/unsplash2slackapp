...
app.post("/select", function(req, res) {
  ...
  if (parsedObject.actions[0].name === "send") {
    ...
    postPicture(
      parseInt(parsedObject.actions[0].value),
      parsedObject.callback_id
    );
...
function successfulResponse(whoSendIt, response, searchWord) {
  ...
  pickAPicture(
    whoSendIt,
    Math.floor(Math.random() * Math.floor(unsplashResponse.length - 1)),
    searchWord
  );
}
function failedResponse(whoSendIt) {
  sendResponse(
    whoSendIt,
    "Oops! We couldn't find anything. You can use your imagination."
  );
}
  ...
  var response = [
    {
      ...
      author_name:
        "Photo by " + unsplashResponse[index].user.name + " on Unsplash",
